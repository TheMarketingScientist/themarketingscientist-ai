...
function Invoke-ArticleValidator {
    ...
    $output = @(& python -B $script:ValidatorPath @arguments 2>&1)
    ...
    try {
        ...
        Stop-Publication (
            ...
            "$($_.Exception.Message)$([Environment]::NewLine)$text"
        )
    }

    if ($exitCode -eq 0 -and $report.status -cne "passed") {
        Stop-Publication "Validator contract error: exit code 0 returned status '$($report.status)'."
    }
    if ($exitCode -ne 0 -and $report.status -cne "failed") {
        Stop-Publication "Validator contract error: exit code $exitCode returned status '$($report.status)'."
    }
...
[void]$allowedAuthoringPaths.Add((Convert-ToRepoPath $script:ArticleFullPath))
...
$unrelatedBefore = @($changesBefore | Where-Object { -not $allowedAuthoringPaths.Contains($_) })
if ($unrelatedBefore.Count -gt 0) {
    Stop-Publication (
        "Unrelated existing changes were found. Only the target article and its referenced source " +
        "images may be changed before generation: $($unrelatedBefore -join ', ')"
    )
}
...
$generationOutput = @(& python "articles/generate_articles.py" 2>&1)
$generationExitCode = $LASTEXITCODE
$generationOutput | ForEach-Object { Write-Host $_ }
$reportedErrors = @($generationOutput | Where-Object { [string]$_ -match "(?i)\berror\b" })
$generationFailed = $generationExitCode -ne 0 -or $reportedErrors.Count -gt 0

$renderArguments = @("--template-sha256", $templateHash)
if (-not $generationFailed) {
    $renderArguments += "--sync-images"
}
$renderedValidation = Invoke-ArticleValidator -Mode "rendered" -AdditionalArguments $renderArguments
Show-ValidatorReport -Report $renderedValidation.Report -Label "Rendered-output validation"

if ($generationExitCode -ne 0) {
    Stop-Publication "Article generation failed with exit code $generationExitCode."
}
if ($reportedErrors.Count -gt 0) {
    Stop-Publication "Article generation reported an error: $($reportedErrors -join [Environment]::NewLine)"
}
